...
- [X] В `erd.md` есть Mermaid-диаграмма со всеми сущностями проекта
- [X] На GitHub диаграмма рендерится (проверьте перед отправкой)
- [X] Каждая связь подписана (что значит стрелка)
- [X] Связи описаны текстом в таблице
- [X] Все связи M:N решены через промежуточные таблицы
- [X] В `entities.md` добавлены FK-поля
- [X] Минимум 3 связи между сущностями
- [X] Нет "висячих" сущностей без связей (каждая сущность связана хотя бы с одной другой)
